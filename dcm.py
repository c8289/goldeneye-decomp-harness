...
Version 3.2 — the Part 2 tripwire is enforced here, not in the prompt.

The operating manual for agents is AGENT.md; README.md is the human-facing
overview. Part references in this file point at AGENT.md.

The harness owns the halt conditions AGENT.md Part 2 describes, because an agent
in a long transcript cannot be trusted to count its own streaks:
  ...
  * 25 builds (AGENT.md checkpoint)                     -> halt
...
# ---- tripwire constants (AGENT.md Part 2) ------------------------------------
...
BUILD_BUDGET = 25       # AGENT.md checkpoint: new_task past 25 iterations
CHECKPOINT_AT = 12      # advisory: recommend a context checkpoint here

# Hypothesis categories, from the AGENT.md Part 5 catalogue.
CATEGORIES = {
...
def git(*args):
    ...
    try:
        proc = subprocess.run(["git", *args], check=False, stdout=subprocess.PIPE,
                              stderr=subprocess.STDOUT, universal_newlines=True)
    except OSError as e:  # git not installed at all
        return 127, str(e)
    return proc.returncode, (proc.stdout or "").strip()


def git_checkpoint(message):
    """Commit the working directory. Warn once, loudly, if git cannot commit.

    A non-zero exit from `git commit` is normal when nothing changed, so that is
    not worth reporting. A repo with no HEAD at all means no commit has ever
    landed - usually an unset user.email - and that silently removes half the
    safety net the manual promises, so it gets said out loud exactly once.
    """
    global _GIT_WARNED
    git("add", "-A")
    code, output = git("commit", "-qm", message)
    if code == 0:
        return True
    if git("rev-parse", "--verify", "-q", "HEAD")[0] == 0:
        return False  # nothing to commit; the repository is healthy
    if not _GIT_WARNED:
        _GIT_WARNED = True
        print("WARNING: git is not checkpointing - best.c is your only safety net.")
        for line in [ln for ln in output.splitlines() if ln.strip()][:4]:
            print("         " + line.strip()[:120])
        print('         usually: git config user.email "you@example.com"'
...
def check_trial(meta, category):
    ...
    if used >= budget:
        return False, ("category '%s' already tried %d times (limit %d) - AGENT.md Part 2: "
...
def apply_build_result(meta, ok, score=None, max_score=None):
    ...
    if meta["flat_streak"] >= FLAT_LIMIT:
        ...
        out["halt"] = ("build budget reached (%d iterations) - AGENT.md Part 2 says checkpoint "
...
def src_lines(side):
    """Normalise the optional `src` annotation, which may be str or list.

    decomp.me returns a single string here; older payloads and some arches
    return a list. Slicing a string with [:1] yields one *character*, which is
    how this printed 'target src: i' for a line reading 'int f(void) {'.
    """
    value = (side or {}).get("src")
    if not value:
        return []
    if isinstance(value, str):
        return [line for line in value.splitlines() if line.strip()]
    if isinstance(value, (list, tuple)):
        return [line if isinstance(line, str) else str(line) for line in value if line]
    return [str(value)]


def marker(row):
    """Classify one diff row from the leading character of the `current` column.

    The API puts the marker in `current`, not `base`, so `base` is only ever a
    fallback. A row whose `current` side is empty while `base` has an
    instruction is a deletion by definition, and its `base` text starts with an
    address rather than a marker - so it must be reported as `<` explicitly
    instead of falling through to "identical".
    """
    current, base = flat(row.get("current")), flat(row.get("base"))
    if current[:1] in MARKERS:
        return current[:1]
    if not current.strip() and base.strip():
        return "<"
    return " "
...
def render(rows, limit, at=None, show_src=False):
    ...
    for i in span:
        ...
        if show_src and marks[i]:
            for source_line in src_lines(row.get("base"))[:1]:
                out.append("        ^ target src: %s" % source_line.strip()[:120])
        previous = i
...
def do_halt(meta, reason, kind, restore):
    ...
    print("\nAGENT.md Part 2 halt procedure — run these, then STOP:")
    print("   ./dcm.py status")
    print("   ./dcm.py hist")
...
def slug_of(text):
    """Accept a bare slug or a full decomp.me URL (AGENT.md Part 1)."""
...
def cmd_trial(args):
    ...
    if len(text) < 8:
        die("state the hypothesis in one sentence - if you cannot, AGENT.md Part 2 says halt")
...
def cmd_build(args):
    ...
    if trial is None:
        if meta["iteration"] == 0:
            ...
            die("no pending hypothesis. AGENT.md Part 5: one hypothesis per compile.\n"
    ...
    if verdict == "IMPROVED":
        shutil.copy(SRC_FILE, BEST_FILE)
        git_checkpoint("iter %d score %d [%s] %s" %
    ...
    if result["halt_kind"] == "match":
        print("\n*** MATCH *** - stop editing.")

    # Every halt is issued *after* the diff is printed, including a regression.
    # do_halt exits the process, and an agent that regressed needs to see what
    # the regression looked like before the workspace is rolled back.
...
def cmd_target(args):
    ...
    shown = min(args.n, MAX_PRINT)
    for line in lines[:shown]:
        print("  " + line[:160])
    if len(lines) > shown:
        print("  ... +%d more (use -n, hard ceiling %d)" % (len(lines) - shown, MAX_PRINT))
...
def cmd_selftest(_args):
    ...
    # 11. diff parsing layer - the part most likely to break when decomp.me
    #     changes its response shape, so it gets fixtures rather than trust.
    def row(base=None, current=None, src=None):
        entry = {}
        if base is not None:
            entry["base"] = {"text": [{"text": base}]}
            if src is not None:
                entry["base"]["src"] = src
        if current is not None:
            entry["current"] = {"text": [{"text": current}]}
        return entry

    check("flat concatenates chunks",
          flat({"text": [{"text": "addiu "}, {"text": "sp,sp,-0x28"}]}) == "addiu sp,sp,-0x28")
    check("flat of missing side", flat(None) == "")
    check("identical row is not a divergence",
          marker(row("4:  sw ra,0x24(sp)", "  4:  sw ra,0x24(sp)")) == " ")
    check("marker read from current column",
          marker(row("0:  addiu sp,sp,-0x28", "i 0:  addiu sp,sp,-0x38")) == "i")
    check("extra instruction", marker(row("", "> 4:  addiu t7,a1,7")) == ">")
    check("missing instruction marked in current",
          marker(row("4:  nop", "< 4:  nop")) == "<")
    check("empty current beside a real target row is a deletion",
          marker(row("4:  addu a1,a2,zero", "")) == "<")
    check("absent current side is a deletion", marker(row("4:  addu a1,a2,zero")) == "<")
    check("blank row either side is not a divergence", marker(row("", "")) == " ")

    rows = [row("0:  addiu sp,sp,-0x28", "i 0:  addiu sp,sp,-0x38"),
            row("4:  sw ra,0x24(sp)", "  4:  sw ra,0x24(sp)"),
            row("8:  sw a0,0x28(sp)", "s 8:  sw a0,0x38(sp)"),
            row("c:  addu a1,a2,zero", "")]
    check("histogram counts each marker once",
          histogram(rows) == {"i": 1, "s": 1, "<": 1})
    check("histogram ignores identical rows", sum(histogram(rows).values()) == 3)
    rendered, count = render(rows, 12)
    check("render reports every divergence", count == 3)
    check("render stays within the print ceiling", len(rendered) <= MAX_PRINT)
    check("render marks the changed row", any(line.startswith(" i ") for line in rendered))
    check("render of a clean diff", render([row("4:  nop", "  4:  nop")], 12)[1] == 0)

    # src annotations arrive as a string; slicing one with [:1] used to print a
    # single character ("target src: i" for "int f(void) {").
    check("src as string", src_lines({"src": "int f(void) {"}) == ["int f(void) {"])
    check("src as list", src_lines({"src": ["a = b;", "c = d;"]}) == ["a = b;", "c = d;"])
    check("src absent", src_lines({}) == [] and src_lines(None) == [])
    annotated, _ = render([row("0:  addiu sp,sp,-0x28", "i 0:  addiu sp,sp,-0x38",
                               src="int f(void) {")], 12, show_src=True)
    check("src annotation prints the whole line",
          any("target src: int f(void) {" in line for line in annotated))

    # 12. CLI wiring. Cheap, but it is the layer that breaks silently: a
    #     subcommand with no handler only fails when someone runs it.
    parser = build_parser()
    documented = ["pull", "trial", "categories", "build", "diff", "hist", "target",
                  "ctx", "family", "revert", "resume", "log", "status", "selftest"]
    commands = parser.get_default("_commands")
    registered = sorted(commands)
    check("every documented command is registered", registered == sorted(documented))
    check("every command has a handler",
          all(commands[name].get_default("f") is not None for name in registered))
    check("no subcommand is handled, not crashed",
          getattr(parser.parse_args([]), "f", None) is None)
...
def build_parser():
    ...
    # `add_subparsers(required=True)` is 3.7+; this file targets 3.6, so the
    # missing-subcommand case is handled after parsing instead.
    ...
    parser.set_defaults(_commands=sub.choices)  # selftest checks this table
    return parser
